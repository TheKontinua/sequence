...
In words, if $g(x)$ is between $f(x)$ and $h(x)$ near $a$ and $f$ and $h$ have the same limit, $L$, then the limit of $g$ must also be $L$. 
...
\begin{tikzpicture}
    ...
    ytick=\empty,
     ...
     \addplot[domain=-.09:0.09, red, samples=100]{(x^2)*sin(deg(1/x))}
...
Then, by the Squeeze Theorem, we can say that: $$\lim_{x \to 0}x^2\sin{\frac{1}{x}}=0$$
...
\begin{Answer}
  ...
  \begin{tikzpicture}
    ...
    \addplot[domain=-0.5:0.5, red, samples=100]{sqrt(x^3+x^2)*cos(deg(1/x))}
...
For an $N$ such that there is more than one $c$ where $f(c) = N$
...
\begin{tikzpicture}
    ...
    extra x tick labels = {$f(a)$, $f(b)$, $c_1$, $c_2$, $c_3$}
    ]
        \addplot[domain = 0:4, samples =50, color=blue]{(x-1.8)^3-2*(x-1.8)+3};
        \addplot[domain=0:3.75, color=green]{6.5};
        \addplot[samples=50,domain=0:6,green] coordinates {(3.75,0)(3.75,6.5)};

...
\begin{Exercise}[label=critval1]
	Let $g$ be the function given by $g(x) = x^2e^{kx}$, where $k$ is a constant. For what value(s) of $k$ does $g$ have a critical value at $x=\frac{2}{3}$?
\end{Exercise}

\begin{Answer}[ref=critval1]
	Recall that critical values are values of $x$ where $g'(x) =0$ or is undefined. We need to find an expression for $g'(x)$, set it equal to zero when $x=\frac{2}{3}$, and solve for $k$. 
	$$g'(x) = x^(2)[k*\exp{kx}]+\exp{kx}[2x]$$
	$$g'(\frac{2}{3})=(\frac{2}{3})^2[k*\exp{\frac{2k}{3}}]+\frac{4}{3}\exp{\frac{2k}{3}}=0$$
...
\begin{figure}[htbp]
	\centering
	\begin{tikzpicture}
		\begin{axis}[xmin=-1.2, xmax=8.5, xtick={-1,0,1,2,3,4,5,6,7,8}, xlabel=$x$, ylabel=$f'(x)$, axis lines = center]
...
[Your answers are meant to be estimates, anything within $\pm 0.1$ of the given answers are reasonable estimates.]
\begin{enumerate}
	...
	\addplot[blue, samples=100, domain=-0.9:7.9]{-4*(x^5)/175+13*(x^4)/35-188*(x^3)/105+2*x^2+3.5*x};
	\end{axis}
	\end{tikzpicture}
\end{enumerate}
\end{Answer}	
